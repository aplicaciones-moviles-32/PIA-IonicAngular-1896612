...
export class RegistroComponent implements OnInit {
  ...
  constructor(private authService: AuthenticationService, private storage: StorageService, public alertController: AlertController) { }
  ...
  linkImagenFirebase:any;

  SubirImagen=(usuario:any,image:any)=>{
    let url;
    return new Promise((resolve)=>{
      this.storage.subirImagen(usuario+"_"+Date.now(),image).then(linkImagen=>{
        this.linkImagenFirebase=linkImagen;
        console.log(this.linkImagenFirebase);
        resolve(this.linkImagenFirebase);
      });  
    })
